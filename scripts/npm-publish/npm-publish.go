...
func main() {
	tagNamePattern := regexp.MustCompile(`(?m)(.+)/v(\d.\d.\d)`)
	...
	if execErr := exec.Command("npm", "publish", "--access", "public", filepath.Join(pluginFolderName, fmt.Sprintf("%s-%s.tar.gz", pluginName, version))).Run(); execErr != nil {
