...
export async function createJob(
  ...
  if (!response.ok) {
    ...
    console.error('Splunk createJob failed:', response.status, response.statusText, errorText);
...
export async function exportSearch(
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error('Splunk exportSearch failed:', response.status, response.statusText, errorText);
      throw new Error(`Splunk API error: ${response.status} ${response.statusText}`);
    }
  } catch (error) {
    console.error('Splunk exportSearch error:', error);
    throw error;
  }
