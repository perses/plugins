...
describe('GreptimeDBTimeSeriesQuery', () => {
  it('should properly resolve variable dependencies', () => {
    if (!GreptimeDBTimeSeriesQuery.dependsOn) throw new Error('dependsOn is not defined');
    const { variables } = GreptimeDBTimeSeriesQuery.dependsOn(
      {
  ...
  it('should create initial options with query', () => {
    const initialOptions = GreptimeDBTimeSeriesQuery.createInitialOptions();
          ...
          output: [
            {
              records: {
                schema: {
                  column_schemas: [{ name: 'count(*)', data_type: 'Int64' }],
                },
                rows: [[44931]],
                total_rows: 1,
              },
