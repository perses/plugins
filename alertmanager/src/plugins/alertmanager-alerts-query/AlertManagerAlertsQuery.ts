...
export const AlertManagerAlertsQuery: AlertsQueryPlugin<AlertManagerAlertsQuerySpec> = {
  ...
  createInitialOptions: (): AlertManagerAlertsQuerySpec => ({
  ...
  dependsOn: (spec: AlertManagerAlertsQuerySpec) => {
    ...
    const receiverVariables = spec.receiver ? parseVariables(spec.receiver) : [];
    return {
      variables: [...new Set([...datasourceVariables, ...filterVariables, ...receiverVariables])],
    };
  },
};
