  ...
  return (
    <TableRow hover>
      {columnDefs.map((col, colIdx) => (
        <TableCell key={colIdx}>{renderSilenceCell(silence, col.name)}</TableCell>
      ))}
      {showActions && (
        <TableCell>
    ...
    <TableContainer sx={{ height: contentDimensions?.height, overflow: 'auto' }}>
      ...
      <Table size="small" stickyHeader>
        <TableHead>
          <TableRow>
            {columnDefs.map((col, colIdx) => (
              <TableCell key={colIdx} sortDirection={sortState?.fieldName === col.name ? sortState.direction : false}>
          ...
          {silences.length === 0 ? (
            <TableRow>
              <TableCell colSpan={99} align="center" sx={{ py: 4 }}>
                <Typography color="text.secondary">No matching silences</Typography>
              </TableCell>
            </TableRow>
          ) : (
