...
export function getGroupSummary(alerts: Alert[], labelKeys?: string[]): GroupSummary {
  ...
  labelCounts[SORT_COL_STATUS] = {};
  labelCounts[SORT_COL_ALERTNAME] = {};
  ...
  for (const alert of alerts) {
    ...
    const status = getEffectiveStatus(alert);
    labelCounts[SORT_COL_STATUS]![status] = (labelCounts[SORT_COL_STATUS]![status] ?? 0) + 1;

    if (alert.name) {
      labelCounts[SORT_COL_ALERTNAME]![alert.name] = (labelCounts[SORT_COL_ALERTNAME]![alert.name] ?? 0) + 1;
    }
