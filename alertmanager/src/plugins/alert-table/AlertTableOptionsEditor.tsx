...
import type { AlertAction, AlertTableOptions } from './alert-table-model';
import { ALL_ALERT_ACTIONS } from './alert-table-model';
...
export function AlertTableOptionsEditor(props: OptionsEditorProps<AlertTableOptions>): ReactElement {
  ...
  return (
    <>
      ...
      <OptionsEditorGroup title="Runbook">
        <TextField
          size="small"
          fullWidth
          value={value.runbookAnnotationKey ?? ''}
          onChange={handleRunbookKeyChange}
          placeholder="runbook_url"
          helperText="Annotation key used to extract the runbook URL. Defaults to runbook_url."
        />
      </OptionsEditorGroup>
    </>
  );
