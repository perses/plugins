...
describe('compareGroupsByColumn', () => {
  ...
  it('sorts groups by status using most critical status', () => {
    const sort: SortState = { columnName: SORT_COL_STATUS, direction: 'asc', mode: 'status' };
    const aCounts = { firing: 3 };
    const bCounts = { pending: 2 };

    expect(compareGroupsByColumn(aCounts, bCounts, sort)).toBeLessThan(0);
  });

  it('sorts groups by alertname alphabetically', () => {
    const sort: SortState = { columnName: SORT_COL_ALERTNAME, direction: 'asc', mode: 'alphabetical' };
    const aCounts = { AlertA: 2 };
    const bCounts = { AlertB: 3 };

    expect(compareGroupsByColumn(aCounts, bCounts, sort)).toBeLessThan(0);
  });
});

describe('compareAlertsByColumn — status sorting', () => {
  it('sorts firing before pending ascending', () => {
    const sort: SortState = { columnName: SORT_COL_STATUS, direction: 'asc', mode: 'status' };
    const a = makeAlert({}, { state: 'firing' });
    const b = makeAlert({}, { state: 'pending' });

    expect(compareAlertsByColumn(a, b, sort)).toBeLessThan(0);
  });

  it('sorts suppressed after firing ascending', () => {
    const sort: SortState = { columnName: SORT_COL_STATUS, direction: 'asc', mode: 'status' };
    const a = makeAlert({}, { state: 'firing', suppressed: true });
    const b = makeAlert({}, { state: 'firing' });

    expect(compareAlertsByColumn(a, b, sort)).toBeGreaterThan(0);
  });

  it('reverses order when descending', () => {
    const sort: SortState = { columnName: SORT_COL_STATUS, direction: 'desc', mode: 'status' };
    const a = makeAlert({}, { state: 'firing' });
    const b = makeAlert({}, { state: 'pending' });
...
describe('compareAlertsByColumn — alertname sorting', () => {
  it('sorts alphabetically by alert name ascending', () => {
    const sort: SortState = { columnName: SORT_COL_ALERTNAME, direction: 'asc', mode: 'alphabetical' };
    const a = makeAlert({}, { name: 'CPUHigh' });
    const b = makeAlert({}, { name: 'MemoryLow' });

    expect(compareAlertsByColumn(a, b, sort)).toBeLessThan(0);
  });

  it('sorts alphabetically by alert name descending', () => {
    const sort: SortState = { columnName: SORT_COL_ALERTNAME, direction: 'desc', mode: 'alphabetical' };
    const a = makeAlert({}, { name: 'CPUHigh' });
    const b = makeAlert({}, { name: 'MemoryLow' });
