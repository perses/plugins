...
export const StatChartPanel: FC<StatChartPanelProps> = (props) => {
  ...
  // Default: one horizontal row (unchanged). Wrap to grid when cells would be too narrow.
  ...
  // Flatten nested ternaries (oxlint no-nested-ternary).
  let justifyContent: 'flex-start' | 'left' | 'center' = 'center';
  if (isMultiSeries) {
    justifyContent = wrap ? 'flex-start' : 'left';
  }
  let overflowX: 'hidden' | 'auto' = 'hidden';
