...
const mockReplaceVariables: jest.Mock = jest.fn((text: string) => text);
jest.mock('@perses-dev/plugin-system', () => ({
  replaceVariables: (...args: unknown[]) => mockReplaceVariables(...args),
}));

import { JsonDatasourceClient } from '../../datasources/json-datasource/json-datasource-types';
import { JsonQuerySpec } from './json-query-types';
import { getJsonData } from './get-json-data';

const makeClient = (): JsonDatasourceClient => ({
  options: { datasourceUrl: 'http://api.example' },
  query: jest.fn(async () => ({ items: ['a', 'b', 'c'] })),
});

function createContext(client: JsonDatasourceClient) {
  return {
    variableState: {},
    datasourceStore: {
...
describe('getJsonData', () => {
  ...
  it('calls client.query with the correct parameters', async () => {
    ...
    await getJsonData(spec, createContext(client) as unknown as Parameters<typeof getJsonData>[1]);
  ...
  it('returns the response from client.query directly when no JSONata expression', async () => {
    ...
    const result = await getJsonData(spec, createContext(client) as unknown as Parameters<typeof getJsonData>[1]);
  ...
  it('forwards query params to client.query', async () => {
    ...
    expect(client.query).toHaveBeenCalledWith(
      expect.objectContaining({ queryParams: { q: 'test', limit: '5' } })
    );
  });
  ...
  it('passes variableState to replaceVariables for endpointUrl interpolation', async () => {
    ...
    mockReplaceVariables.mockImplementation((text: string) =>
      text === '/api/$env/data' ? '/api/prod/data' : text
    );
  ...
  it('passes variableState to replaceVariables for each query param value', async () => {
    ...
    expect(client.query).toHaveBeenCalledWith(
      expect.objectContaining({ queryParams: { team: 'ops', env: 'prod' } })
    );
  });
  ...
  describe('JSONata transformation', () => {
    it('applies JSONata expression to the response', async () => {
      const client = makeClient();
      client.query = jest.fn(async () => ({ users: [{ name: 'Alice' }, { name: 'Bob' }] }));
    ...
    it('throws when JSONata expression matches nothing', async () => {
      const client = makeClient();
      client.query = jest.fn(async () => ({ data: [] }));
      const spec: JsonQuerySpec = { endpointUrl: '/data', method: 'GET', jsonataExpression: 'nonexistent.path' };

      await expect(
        getJsonData(spec, createContext(client) as unknown as Parameters<typeof getJsonData>[1])
      ).rejects.toThrow('JSONata expression did not match any data');
    });

    it('returns raw response when jsonataExpression is not set', async () => {
      const client = makeClient();
      const raw = { value: 99 };
      client.query = jest.fn(async () => raw);
      const spec: JsonQuerySpec = { endpointUrl: '/data', method: 'GET' };
